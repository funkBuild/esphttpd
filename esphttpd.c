  ...
  while (offset < body_len) {
    ...
    if (var_found && var_end > var_start) {
      ...
      if (replacement != NULL) {
        ...
        send(req->sock, body + offset, prefix_len, 0);
        send(req->sock, replacement, strlen(replacement), 0);
...
void webserver_send_file_response(http_req* req, char* file_path, char* content_type) {
  ...
  while ((n = fread(buf, 1, SEND_BUFFER_SIZE, f)) != 0 && send(req->sock, buf, n, 0) != -1) {
  }
...
static void webserver_ws_task(void* pvParameter) {
  ...
  while ((length = recv(ctx->sock, (void*)&frame_header, 2, 0)) == 2) {
    ...
    ws_event event = {.event_type = WS_MESSAGE, .payload = NULL, .len = frame_header.payload_length};
    ...
    if (event.len > 0) {
      ...
      if (frame_header.mask) {
        for (int i = 0; i < event.len; i++) event.payload[i] ^= masking_key[i % WS_MASK_LEN];
      }
    }
